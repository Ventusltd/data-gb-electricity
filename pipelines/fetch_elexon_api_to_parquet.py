#!/usr/bin/env python3
"""Elexon API to partitioned Parquet updater for data-gb-electricity.

This is the new federated data process: fetch from Elexon, normalise in this
repo, merge by stable keys, and write compact zstd Parquet partitions directly.
It does not use the retiring monolith and it never writes raw CSV.
"""
...
def parse_dt(value: Any) -> dt.datetime | None:
    ...
    if isinstance(value, dt.datetime):
        ...
        d = dt.datetime.combine(value, dt.time(0, 0), tzinfo=dt.timezone.utc)
    else:
...
def period_start_from_date_period(date_text: str, period: int | None) -> dt.datetime | None:
    if period is None:
        return None
    try:
        return dt.datetime.fromisoformat(date_text).replace(tzinfo=dt.timezone.utc) + dt.timedelta(minutes=(period - 1) * 30)
    except Exception:
        return None
...
def fetch_fuelinst(start: dt.date, end: dt.date, window_days: int, retries: int, delay: float) -> list[dict[str, Any]]:
    ...
    for w_start, w_end in windows(start, end, window_days):
        start_dt = dt.datetime.combine(w_start, dt.time(0, 0), tzinfo=dt.timezone.utc)
        end_dt = dt.datetime.combine(w_end, dt.time(23, 59), tzinfo=dt.timezone.utc)
...
def write_reports(payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Elexon API to Parquet Run",
        "",
        f"Updated UTC: {payload['updatedUTC']}",
        f"Apply: {payload['apply']}",
        f"Date range: {payload['startDate']} to {payload['endDate']}",
        f"Datasets: {', '.join(payload['datasets'])}",
        "",
        "## Results",
    ]
    for result in payload["results"]:
        lines.append(f"- {result['dataset']}: fetched {result['rowsFetched']} rows; touched {result['monthsTouched']} month partition(s)")
    lines.extend(["", "## Source endpoints", "", f"- FUELINST: `{FUELINST_URL}`", f"- FUELHH: `{FUELHH_URL}`", f"- PRICES: `{SYSTEM_PRICE_URL}/YYYY-MM-DD`"])
    (reports / "elexon_api_to_parquet_latest.md").write_text("\n".join(lines) + "\n", encoding="utf-8")


def main() -> int:
    parser = argparse.ArgumentParser(description="Fetch Elexon API data and write partitioned Parquet")
    parser.add_argument("--start-date", help="Inclusive start date YYYY-MM-DD. Defaults to first day of previous calendar month.")
    parser.add_argument("--end-date", help="Inclusive end date YYYY-MM-DD. Defaults to last day of previous calendar month.")
    ...
    parser.add_argument("--apply", action="store_true", help="Write Parquet. Without this, only fetch/report what would change.")
    ...
    yesterday = utc_now_dt().date() - dt.timedelta(days=1)
    ...
    payload = {
        "schemaVersion": "elexon_api_to_parquet.v1",
