...
def sources(root: Path) -> dict[str, list[str]]:
    return {
        "fuelinst": sorted(glob.glob(str(root / "data/generation/archive/*/*.csv"))),
        "fuelhh": sorted(glob.glob(str(root / "data/generation/fuelhh_halfhourly/*/*.csv"))),
        "prices": sorted(glob.glob(str(root / "data/electricity/elexon_system_prices_*.csv"))),
    }
...
def convert(src_root: Path, out_root: Path) -> None:
    ...
    con.execute(f"""COPY (SELECT *, 'fuelinst' AS dataset, year(periodStartUTC) AS year, month(periodStartUTC) AS month
      FROM read_csv_auto({fi!r}, union_by_name=true))
      TO '{out}/generation/dataset=fuelinst' (FORMAT parquet, COMPRESSION zstd, PARTITION_BY (year,month), OVERWRITE_OR_IGNORE);""")

    con.execute(f"""COPY (SELECT *, 'fuelhh' AS dataset, year("time") AS year, month("time") AS month
      FROM read_csv_auto({fh!r}, union_by_name=true))
      TO '{out}/generation/dataset=fuelhh' (FORMAT parquet, COMPRESSION zstd, PARTITION_BY (year,month), OVERWRITE_OR_IGNORE);""")

    con.execute(f"""COPY (SELECT *, year(periodStartUTC) AS year, month(periodStartUTC) AS month
      FROM read_csv_auto({pr!r}, union_by_name=true))
...
def main() -> int:
    ...
    src = sources(src_root)
    check_counts(src)
    print("source MB:", {k: round(sum(Path(f).stat().st_size for f in v) / 1048576, 3) for k, v in src.items()})
